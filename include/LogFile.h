...
/**
 * @brief 日志文件管理类
 * 负责日志文件的创建、写入、关闭(封装了FileUtil) | 滚动和刷新(LogFile新增)等操作
 * 支持按大小和时间自动滚动日志文件
 */
class LogFile
{
...
private:
    ...
    const off_t rollsize_;          // 日志文件大小达到多少字节时滚动，单位:字节
    const int flushInterval_;       // 日志刷新间隔时间，默认3秒
    ...
    const static int kRollPerSeconds_ = 60*60*24;       //滚动日志时间周期
};
