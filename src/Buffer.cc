...
ssize_t Buffer::readFd(int fd, int *saveErrno)
{
    ...
    */
    ...
    // 如果(基于经验地认为)空间足够(writable>64k),则不使用extrabuffer。如果使用extrabuf,则最多使用128k-1的空间
    const int iovcnt = (writable < sizeof(extrabuf)) ? 2 : 1;
    const ssize_t n = ::readv(fd, vec, iovcnt);             //▲ iovcnt决定readv【顺序使用】的缓冲区个数
    ...
    else // extrabuf里面也写入了n-writable长度的数据
    {
        ...
        Buffer::append(extrabuf, n - writable); // 对buffer_扩容 并将extrabuf存储的另一部分数据追加至buffer_
    }
    return n;
}

// inputBuffer_.readFd表示将对端数据读到inputBuffer_中，移动writerIndex_指针
// outputBuffer_.writeFd表示将本端数据写入到fd中，从readerIndex_开始，可以写readableBytes()个字节
...
/*
inputBuffer_和outputBuffer_的区别：
	
inputBuffer_    网络 → Buffer → 应用程序    socket 可读时（对端有数据）
outputBuffer_   应用程序 → Buffer → 网络    应用程序调用 send()时
*/