...
const int kPollTimeMs = -1; // 10000毫秒 = 10秒钟
...
EventLoop::~EventLoop()
{
    ...
    ::close(wakeupFd_);           // ::表示是全局命名空间的close函数
...
void EventLoop::loop()
{
    ...
    while (!quit_)
    {
         ...
         **/
        doPendingFunctors();    // 处理任务队列(线程内部通信任务+非当前loop所属线程欲执行cb)
    }
 ...
 * 2. 如果不是当前EventLoop所属线程中调用quit退出EventLoop 需要唤醒EventLoop所属线程的epoll_wait(避免无限阻塞)
 *
...
{
    if (isInLoopThread()) // 当前EventLoop的所属线程执行回调(允许)
    {
        cb();
    }
    else // 在非当前EventLoop线程中执行cb(不允许) 就需要使用queueInLoop将cb加入当前eventloop的任务队列 然后唤醒EventLoop所属线程执行cb
    {
...
{
    {
        //为什么要加锁：因为其他的线程也可能向当前eventloop的任务队列添加cb
        std::unique_lock<std::mutex> lock(mutex_);  //大括号创建局部作用域 控制锁的作用域 -> 大括号结束后 锁自动析构
...
// 用来唤醒当前loop所在线程 向wakeupFd_写一个数据 wakeupChannel就发生读事件 当前loop线程就会被唤醒
...
// 处理内部线程间通信任务 (如：1.添加新连接、关闭连接等内部管理任务 2.非当前loop所属线程欲执行cb 但不可 故将cb放入任务队列 唤醒当前loop的所属线程)
